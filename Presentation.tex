...
\begin{frame}
\frametitle{Households with 3 Members \only<-5>{and 1 Child} \only<6->{after Last Reform}}
\begin{center}
\scalebox{0.7}{
...
\pause
\only<-5>{\draw[red!70!black, -, very thick] (0,30) -- (70,100) ; % 45 degree line
...
\pause
\only<2-5>{\draw[green!70!black, -, very thick] (0,34) -- (70,104) ; % 45 degree line
...
\pause
\only<3-5>{\draw[blue!70!black, -] (0,70) -- (36,70) ; % flat line
...
\pause
\only<5->{\draw[black, -] (0,77) -- (39.666,77) ; % flat line
...
\node[black, align=center, right] at (180,110) {\only<5>{06.01.2014}\only<6->{With Children}};
...
\pause
\pause
\only<6->{\draw[orange, -] (0,77) -- (51.333,77) ; % flat line
...
\pause
\begin{itemize}
\item Bolsa Fam\'ilia Administrative Data for 2012-2015 (only 2015 today)
\begin{itemize}
...
\begin{frame}
\frametitle{Reduced-Form Bunching}
\begin{center}
\begin{tikzpicture}[x=.9,y=.9]
\node[align=center, right] at (200,0) {$\bar{y}$};
\draw[->, very thick] (0,0) -- (200,0) ; %edit here for the x axis
...
\only<1>{\draw [decorate,decoration={brace,amplitude=1pt},xshift=-2pt,yshift=0pt]
...
\draw[black, -, dashed, thick] (77,0) -- (77,88.666) ;
\node[black, align=center, below] at (77,0) {\tiny{$\bar{y}^*$}};

\pause
\draw [green!40!black, thick] plot [smooth, tension=.8] coordinates { (36,89.666) (77,114.333) (96,149.666)}; %Low type
\node[align=center, above] at (96,149.666) {L};

\pause
\draw [green!40!black, thick] plot [smooth, tension=.8] coordinates {(57,111.333)  (120,131.666) (150,199)}; %High type with the notch
\node[align=center, above] at (150,199) {H};
\draw[black, -, dashed, thick] (120,0) -- (120,131.666);
\node[black, align=center, below] at (125,0) {\tiny{$\bar{y}_I$}};

\pause 
\draw [green!40!black, thick] plot [smooth, tension=.8] coordinates {(47,131)  (110,147.333) (132,211.666)}; %High type without the notch
\draw[black, -, dashed, thick] (110,0) -- (110,147.333) ;
\node[black, align=center, below] at (107,0) {\tiny{$\bar{y}^*$+$\Delta \bar{y}^*$}};

\pause 
\only<5-7>{\draw[black, -, dashed, thick] (110,0) -- (110,147.333) ;
\node[black, align=center, below] at (107,0) {\tiny{$\bar{y}^*$+$\Delta\bar{y}^*$}};
\draw[black, -, dashed, thick] (77,114.333) -- (180,135.853);
\node[black, align=center, right] at (140,98){\scriptsize{$e\cong -\frac{1}{\bar{y}^*}\frac{\Delta \bar{y}^*}{\Delta b_h}$}};
\node[black, align=center, right] at (140,83){\scriptsize{Where $\Delta b_h=\frac{B(\bar{y}^*+\Delta \bar{y}^*)-B(\bar{y}^*)}{\Delta \bar{y}^*}$}};} %hypothetical kink

\pause
\only<6>{\draw[black, -, dashed, thick] (90,0) -- (90,115) ;
\node[black, align=center, below] at (90,0) {\tiny{$\bar{y}_{h}$}};}

\pause
\only<7>{\draw[black, -, dashed, thick] (0,98.245) -- (77,114.333);
\draw[decorate,decoration={brace,amplitude=1pt},xshift=-2pt,yshift=0pt]
(0,37.333) -- (0,98.245)node [black,midway,xshift=-15pt] {\footnotesize $\Delta \bar{y}^V$};}
...
\pause
\node[black, align=center, right] at (140,120){\scriptsize{ $u^H(\bar{y}^*;e,\tilde{e})=u^H(\bar{y}^*+\Delta \bar{y}^*;e,\tilde{e})$}};
\pause
\node[black, align=center, right] at (140,108){\scriptsize{$\Rightarrow$ If I could get $\bar{e}$, I could get $e$ and $\tilde{e}$}};
...
\pause
\node[black, align=center, right] at (148,120){\scriptsize{$u^H(y^*;e,\tilde{e})=u^H(y^*+\Delta y^*;e,\tilde{e})$}};
\pause
\node[black, align=center, right] at (148,108){\scriptsize{\textcolor{red}{$y^*$ and $\Delta y^*$ depends on $\tilde{y}(y)$}}};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{itemize}
\item $\bar{y}_{jt}$: Income reported by household $j$ in period $t$.
\pause
\item $\bar{y}^*_{jt}$: Latent reported income, so that $\bar{y}_{jt}=\begin{cases}\bar{y}^*_{jt}\ if\ \bar{y}^*_{jt}>0\\
0\ if\ \bar{y}^*_{jt}\leq 0 \end{cases}$
\pause
\begin{center}
\scalebox{0.65}{
...
0 & \text{otherwise.} \end{cases}$  ``intrinsic" income level
\pause
\item $h_{ijt}=\begin{cases}1\ & \text{hh $j$ reports $i$ in $t$},\\
0 & \text{otherwise.} \end{cases}$ ``equilibrium" income level
\end{itemize}
\end{frame}
...
\end{itemize}
\hyperlink{proof_empirical}{\beamergotobutton{Link to Proof}}
\pause
\begin{block}
...
\begin{itemize}
\pause
\item Recover elasticities of reported and real income from bunching and reforms variation
\pause
